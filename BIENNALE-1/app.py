...
# Menu in alto
vocidimenu = [{'caption': 'Notizie', 'href': 'notizie'},
              {'caption': 'Eventi', 'href': 'eventi'},
...
def home():
    # Database
    con = sql.connect("dbbiennale.sqlite")
    ...
    contenuto = {'title': 'La Biennale Di Venezia', 'navigation': vocidimenu}
...
@app.before_request
def before_request():
    g.conn = sql.connect("dbbiennale.sqlite")
...
@app.route("/admin-eventi/<username>")
def admin_eventi(username):
...
@app.route("/admin-notizie/<username>")
def admin_notizie(username):
    if not session.get(username):
        abort(401)
